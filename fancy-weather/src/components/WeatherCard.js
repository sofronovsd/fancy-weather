            ...
            <Card.Body>
                ...
                <div className="flex-container">
                    <p className="card__degree">
                        {data.temp && (isC ? Math.floor(data.temp.value) : Math.floor((data.temp.value * 9) / 5 + 32))}
                        <sup>&deg;{isC ? `C` : `F`}</sup>
                    </p>
                    <div className="card__summary text text__medium">
                        <p>{data.weather_code && data.weather_code.value.replace('_', ' ')}</p>
                        <p>Feels like: {data.feels_like && (isC ? Math.floor(data.feels_like.value) : Math.floor((data.feels_like.value * 9) / 5 + 32))}<sup>&deg;{isC ? `C` : `F`}</sup></p>
